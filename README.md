...
Запустите !start-zapret-gui.bat, выберите пункт Update hosts file. После чего, если ваш hosts будет неактуальным, то Вам будет предложено обновить его самостоятельно:
    ...
    Откройте файл hosts в появившейся папке с помощью текстового редактора от имени администратора
...
## Как переустановить/обновить полностью?
    ...
    !start-zapret-gui.bat -> Run Diagnostics (если есть ошибки - устраните их)
    Удалите папку с запретом
    Скачайте последнюю версию со страницы релизов (zapret-GUI...)
    ...
    Далее протестируйте все конфиги!start-zapret-gui.bat -> Run tests -> (ввод) 1 -> (ввод) 1.
